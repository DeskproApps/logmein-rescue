  ...
  SaxParser.prototype._parseLoop = function(parser) {
    ...
    while(!this.m_bErr) {
      ...
      if(iEvent == XMLP._ELM_B) {
        var theatts = this.m_parser.m_atts;
        var nameobject = parser._parsePrefixAndElementName(parser.getName());
        var theattsandnamespace = parser._parseNamespacesAndAtts(theatts);
