...
WSDL.prototype.objectToXML = function(obj, name, namespace, xmlns) {
    var self = this,
        parts = [],
        xmlnsAttrib = (namespace && xmlns) ? ' xmlns:'+namespace+'="'+xmlns+'"'+' xmlns="'+xmlns+'"' : '',
...
function open_wsdl(fetch, uri, options, callback) {
    ...
    http.request(fetch, uri, null /* options */, function (err, response, definition) {
        if (err) {
            callback(err);
        }
